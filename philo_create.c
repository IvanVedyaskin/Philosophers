...
int	is_died(t_philos *philos, t_main_philo *philo_main)
{
	...
	while (i < philo_main->num_of_philo)
	{
		...
		{
			printf ("philo is died № %ld\n", (philo_main->start_time.tv_sec - philos[i].eating_time.tv_sec) / 1000 + (philo_main->start_time.tv_usec - philos[i].eating_time.tv_usec) * 1000);
			printf ("1 = %ld\n", philo_main->start_time.tv_sec);
